...
from wars_game.abilities.construct import NeedsConstructOrRepair
...
class UnitStalker(BaseClass):    
    ...
    def Precache(self): 
        ...
        super(UnitStalker, self).Precache()
    
    def Spawn(self):
        super(UnitStalker, self).Spawn()
    ...
    if isserver:
        def UpdateOnRemove(self):
            # ALWAYS CHAIN BACK!
            super(UnitStalker, self).UpdateOnRemove()
            self.KillAttackBeam()
        
        def Event_Killed(self, info):
            self.KillAttackBeam()
            super(UnitStalker, self).Event_Killed(info)
            
        def StartRangeAttack(self):
            #if not self.beam:
            #    self.StartAttackBeam(self.enemy)
            #else:
            self.beamtarget = self.enemy
            return False
        ...
        def CalcBeamPosition(self):
            ...
            if self.beamtarget:
                ...
                noiseScale = atan(0.2/fTargetDist)
        ...
        def StartAttackBeam(self, target):
            ...
            if target.IsUnit():
                ...
                self.laserdir = target.WorldSpaceCenter()  - self.LaserStartPosition(self.GetAbsOrigin())
            ...
            if not self.beam:
                ...
                if self.beampower is self.STALKER_BEAM_LOW:
                    ...
                    self.lightglow = CSprite.SpriteCreate( "sprites/redglow1.vmt", self.GetAbsOrigin(), False )
                elif self.beampower is self.STALKER_BEAM_MED:
                    #self.beam.SetColor( 255, 50, 0 )
                    self.beam.SetColor(color.r(), color.g(), color.b())
                    self.lightglow = CSprite.SpriteCreate( "sprites/orangeglow1.vmt", self.GetAbsOrigin(), False )
                elif self.beampower is self.STALKER_BEAM_HIGH:
                    #self.beam.SetColor( 255, 150, 0 )
                    self.beam.SetColor(color.r(), color.g(), color.b())
                    self.lightglow = CSprite.SpriteCreate( "sprites/yellowglow1.vmt", self.GetAbsOrigin(), False )
        ...
        def DrawAttackBeam(self, interval):
            ...
            # Recompute laser dir
            self.laserdir = self.beamtarget.BodyTarget(self.EyePosition()) - self.LaserStartPosition(origin)
            ...
            if pBCC and (pBCC.IsUnit() or pBCC.IsPlayer()):
                ...
                if self.playinghitwall:
                    self.StopSound(self.beam.entindex(), "NPC_Stalker.BurnWall")
            ...
            if not pBCC:# or pBCC.Classify() == CLASS_BULLSEYE:
                ...
                if self.playinghitflesh:
                    self.StopSound(self.beam.entindex(), "NPC_Stalker.BurnFlesh" )
    ...
    if isserver:
        class BehaviorGenericClass(BaseClass.BehaviorGenericClass):
            class ActionAttack(BaseClass.BehaviorGenericClass.ActionAttack):
                def OnStart(self):
                    self.outer.StartAttackBeam(self.outer.enemy)
                    return super(UnitStalker.BehaviorGenericClass.ActionAttack, self).OnStart() 
                    
                def Update(self):
                    self.outer.UpdateAttackBeam(self.outer.enemy)
                    return super(UnitStalker.BehaviorGenericClass.ActionAttack, self).Update() 
                    
                def OnEnd(self):
                    self.outer.KillAttackBeam()
                    return super(UnitStalker.BehaviorGenericClass.ActionAttack, self).OnEnd() 

            class ActionOrderAttack(BaseClass.BehaviorGenericClass.ActionOrderAttack):
                def OnStart(self):
                    self.outer.StartAttackBeam(self.outer.enemy)
                    return super(UnitStalker.BehaviorGenericClass.ActionOrderAttack, self).OnStart() 
                    
                def OnEnd(self):
                    self.outer.KillAttackBeam()
                    return super(UnitStalker.BehaviorGenericClass.ActionOrderAttack, self).OnEnd() 
            ...
            class ActionConstruct(BaseClass.BehaviorGenericClass.ActionConstruct):
                def Update(self):
                    if self.constructing:
                        ...
                        self.outer.KillAttackBeam()
                    return super(UnitStalker.BehaviorGenericClass.ActionConstruct, self).Update()
                    
                def OnEnd(self):
                    self.outer.KillAttackBeam()
                    return super(UnitStalker.BehaviorGenericClass.ActionConstruct, self).OnEnd()
            ...
            class ActionRepair(BaseClass.BehaviorGenericClass.ActionRepair):
                def Update(self):
                    if self.constructing:
                        ...
                        self.outer.KillAttackBeam()
                    return super(UnitStalker.BehaviorGenericClass.ActionRepair, self).Update()
                    
                def OnEnd(self):
                    self.outer.KillAttackBeam()
                    return super(UnitStalker.BehaviorGenericClass.ActionRepair, self).OnEnd() 
    ...
    MAX_STALKER_FIRE_RANGE = 3600 # 3600 feet.
...
class StalkerShared(UnitInfo):
    ...
    image_name = 'vgui/units/unit_stalker.vmt'
...
class StalkerInfo(StalkerShared):
    ...
    abilities = {
        ...
        9 : 'holdposition',
        10 : 'construct',
        11 : SubMenu(name='stalker_menu', 
                     displayname='#CombMenu_Name', description='#CombMenu_Description', 
                     abilities={
            0 : 'build_comb_hq',
            1 : 'build_comb_powergenerator',
            2 : 'build_comb_garrison',
            3 : 'build_comb_regenerationpost',
            4 : 'comb_mountableturret',
            5 : 'build_comb_armory',
...
class StalkerOverrunInfo(StalkerShared):
    ...
    abilities = {
        ...
        4 : 'overrun_comb_mountableturret',
        8 : 'attackmove',
        9 : 'holdposition',
        10 : 'construct',
    }
