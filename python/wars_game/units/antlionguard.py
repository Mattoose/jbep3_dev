...
if isserver:
    ...
    class CTraceFilterCharge(CTraceFilterEntitiesOnly):
        def __init__(self, passentity, collisionGroup, pAttacker):
            super(CTraceFilterCharge, self).__init__()
...
class UnitAntlionGuard(BaseClass):
    """ Antlion Guard """
    def __init__(self):
        super(UnitAntlionGuard, self).__init__()
        
        self.caveglow = [None, None]

    def Precache(self):
        super(UnitAntlionGuard, self).Precache() 
    ...
    def Spawn(self):
        if isclient:
            super(UnitAntlionGuard, self).Spawn()
            return
    ...
    def UpdateOnRemove(self):
        ...
        super(UnitAntlionGuard, self).UpdateOnRemove()
    ...
    def OnChangeOwnerNumber(self, oldownernumber):
        super(UnitAntlionGuard, self).OnChangeOwnerNumber(oldownernumber)
    ...
    def Event_Killed(self, info):
        super(UnitAntlionGuard, self).Event_Killed(info)
    ...
    def HandleChargeImpact(self, vecImpact, pEntity):
        ...
        if self.IRelationType( pEntity ) == D_HT and ( self.GetNextAttack() < gpGlobals.curtime ):
            self.EmitSound( "NPC_AntlionGuard.Shove" )
        ...
        if pEntity.GetMoveType() == MOVETYPE_VPHYSICS:
            ...
            if pPhysics:
                ...
                if not pPhysics.IsMoveable() or pPhysics.GetMass() > VPhysicsGetObject().GetMass() * 0.5:
                    return 1
    ...
    if isclient:
        def OnDataChanged(self, type):
            super(UnitAntlionGuard, self).OnDataChanged(type)
        ...
        # def ClientThink(self):
            ...
            # super(UnitAntlionGuard, self).ClientThink()
        # dlight = None
    ...
    if isserver:
        ...
        aetable = {
            ...
            'AE_ANTLIONGUARD_VOICE_BARK' : None,
            'AE_ANTLIONGUARD_VOICE_PAIN' : None,
            'AE_ANTLIONGUARD_VOICE_SQUEEZE' : EmitSoundAnimEventHandler('NPC_AntlionGuard.Anger'),
            'AE_ANTLIONGUARD_VOICE_SCRATCH' : None,
            'AE_ANTLIONGUARD_VOICE_GRUNT' : None,
            'AE_ANTLIONGUARD_BURROW_OUT' : None,
    ...
    class AnimStateClass(BaseClass.AnimStateClass):
        def OnNewModel(self):
            super(UnitAntlionGuard.AnimStateClass, self).OnNewModel()
        ...
        def OnEndSpecificActivity(self, specificactivity):
            ...
            return super(UnitAntlionGuard.AnimStateClass, self).OnEndSpecificActivity(specificactivity)

    # Antlion Guard AI
    if isserver:
        class BehaviorGenericClass(BaseClass.BehaviorGenericClass):
            class ActionPreChargeMove(BaseClass.BehaviorGenericClass.ActionAbility):
                ...
                def Update(self):
                    ...
                    if dist > 1024:
                        return self.SuspendFor(self.behavior.ActionMoveInRange, 'Not in range of target', target, 1024, 0.0, 128.0)
                        
                    # Facing?
                    if not self.outer.FInAimCone(targetpos, 0.86):
                        return self.SuspendFor(self.behavior.ActionFaceTarget, "Not facing target", target, 0.86)
                        
                    return self.ChangeTo(self.behavior.ActionChargeMove, "In range and facing target", self.order)
                        
            class ActionChargeMove(BaseClass.BehaviorGenericClass.ActionAbility):
                def OnStart(self):
                    # Take movement control
                    self.oldyawspeed = self.outer.mv.yawspeed
                    self.outer.mv.yawspeed = 4 # Make turning very slow
                    self.outer.navigator.StopMoving() # Make sure we are not already moving
                    self.outer.aimoving = True
                    self.outer.DoAnimation(self.outer.ANIM_STARTCHARGE)
                    
                def OnEnd(self):
                    # Give back movement control to locomotion component
                    self.outer.aimoving = False
                    self.outer.mv.maxspeed = self.outer.maxspeed
                    self.outer.mv.yawspeed = self.oldyawspeed
                    
                def Update(self):
                    self.outer.ChargeLookAhead()
                    
                    # GetIdealSpeed corresponds to the current sequence speed
                    self.outer.mv.forwardmove = self.outer.GetIdealSpeed()
                    self.outer.mv.maxspeed = self.outer.GetIdealSpeed()
                    
                    if self.outer.mv.blocker:
                        impacttype = self.outer.HandleChargeImpact(self.outer.GetAbsOrigin(), self.outer.mv.blocker)
                        if impacttype == 1:
                            self.chargeending = True
                            self.outer.ClearOrder(dispatchevent=False)
                            self.outer.DoAnimation(self.outer.ANIM_CRASHCHARGE)
