from srcbase import (MOVETYPE_FLY, SOLID_BBOX, FSOLID_TRIGGER, FSOLID_NOT_SOLID, COLLISION_GROUP_NONE, MASK_SHOT, MASK_SOLID,
...
from fields import FloatField, BooleanField
...
if isserver:
    ...
    from utils import UTIL_SetSize, UTIL_EntitiesInBox, UTIL_ScreenShake, ShakeCommand_t, CTraceFilterSkipFriendly
...
if isclient:
    def DispelCallback(data):
        ...
        endPos = data.origin + Vector(0,0,-64)
...
class UnitVortigaunt(BaseClass):   
    ...
    maxspeed = FloatField(value=226.0)
    energyregenrate = 3
    ...
    if isserver:
        def __init__(self):
            super(UnitVortigaunt, self).__init__()
            
            self.handeffect = [None, None] 

        def Spawn(self):
            super(UnitVortigaunt, self).Spawn()
        ...
        def Precache(self):
            super(UnitVortigaunt, self).Precache()
        ...
        if isserver:
            def UpdateOnRemove(self):
                super(UnitVortigaunt, self).UpdateOnRemove()
        ...
        def ZapBeam(self, nHand):
            ...
            pEntity = tr.ent
            if pEntity is not None and self.takedamage:
                if g_debug_vortigaunt_aim.GetBool():
                ...
                if pEntity.GetClassname() == "unit_antlion":
        ...
        def ClearHandGlow(self):
            ...
            if self.handeffect[0] != None:
                UTIL_Remove( self.handeffect[0] )
                self.handeffect[0] = None

            if self.handeffect[1] != None:
        ...
        def DispelAntlions(self, vecOrigin, flRadius, bDispel=True):
            ...
            for antlion in enemies:
                # We only care about antlions
                #if antlion.GetClassname() != 'unit_antlion': #IsAntlion( antlion ) == False:
                #    continue
                #if antlion.IsWorker() == False:
                # Attempt to trace a line to hit the target
                filter = CTraceFilterSkipFriendly(self, COLLISION_GROUP_NONE, self)
                tr = trace_t()
                UTIL_TraceLine(vecOrigin, antlion.BodyTarget(vecOrigin), MASK_SOLID_BRUSHONLY, filter, tr)
                if tr.fraction < 1.0 and tr.ent != antlion:
                    continue
                vecDir = (antlion.GetAbsOrigin() - vecOrigin)
                vecDir[2] = 0.0
                ...
                antlion.ApplyAbsVelocityImpulse(vecDir)

                # gib nearby antlions, knock over distant ones. 
                if flDist < 200 and bDispel and self.IRelationType(antlion) == D_HT:
                    # splat!
                    vecDir[2] += 400.0 * flFalloff
                    dmgInfo = CTakeDamageInfo(self, self, vecDir, antlion.GetAbsOrigin() , 100, self.unitinfo.AttackRange.damagetype)
                    antlion.TakeDamage(dmgInfo)
                else:
                    # Turn them over
                    try:
                        antlion.Flip(True)
                    ...
                    # Tell the client to start an arm beam
                    '''
                    unsigned char uchAttachment = antlion.LookupAttachment( "mouth" )
                    EntityMessageBegin( self, True )
                        WRITE_BYTE( VORTFX_ARMBEAM )
                        WRITE_LONG( antlion.entindex() )
        ...
        def ReceiveMessage(self, msg):
            ...
            if messageType is self.VORTFX_ZAPBEAM:
                ...
                effect = self.ParticleProp().Create( "vortigaunt_beam", ParticleAttachment_t.PATTACH_POINT_FOLLOW, attachment )
                if effect:
                ...
                nIndex = msg[1]
                ent = ClientEntityList().GetBaseEntityFromHandle( ClientEntityList().EntIndexToHandle( nIndex ) )
                ...
                if ent:
                    ...
                    effect = ent.ParticleProp().Create( "vortigaunt_beam_charge", ParticleAttachment_t.PATTACH_POINT_FOLLOW, attachment )
                    if effect:
    ...
    if isserver:
        # Anim events
        aetable = {
            'AE_VORTIGAUNT_CLAW_LEFT' : None,
            'AE_VORTIGAUNT_CLAW_RIGHT' : None,
            ...
            'AE_VORTIGAUNT_SWING_SOUND' : None,
...
class VortigauntEffectDispel(BaseDispelEffect):
    ...
    if isserver:
        def Spawn(self):
            ...
            UTIL_SetSize( self, Vector( -8, -8, -8 ), Vector( 8, 8, 8 ) )
        ...
        def NotifyShouldTransmit(self, state):
            ...
            if state == SHOULDTRANSMIT_START:
                self.effect = self.ParticleProp().Create( "vortigaunt_hand_glow", ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW )
        ...
        def SetupEmitters(self):
            ...
            self.dlight.origin = GetAbsOrigin()
...
class VortigauntInfo(UnitInfo):
    ...
    image_name = 'vgui/units/unit_vort.vmt'
    costs = [('requisition', 12), ('scrap', 6)]
    buildtime = 60.0
    health = 450
    unitenergy = 150
