...
class UnitHeadcrabCanister(BaseClass):
    ...
    if isclient:
        ...
        def OnDataChanged(self, updatetype):
            super(UnitHeadcrabCanister, self).OnDataChanged(updatetype)
    ...
    if isserver:
        def Precache(self):
            super(UnitHeadcrabCanister, self).Precache()
        ...
        def Spawn(self):
            super(UnitHeadcrabCanister, self).Spawn()
        ...
        def UpdateOnRemove(self):
            """ On remove! """
            super(UnitHeadcrabCanister, self).UpdateOnRemove()
        ...
        def HeadcrabCanisterSpawnHeadcrabThink(self):
            ...
            if self.GetAttachment(nHeadCrabAttachment, vecSpawnPosition, vecSpawnAngles):
                pHeadCrab = CreateUnitNoSpawn(self.headcrabclass[self.headcrabtype], owner_number=self.GetOwnerNumber())
        ...
        def Detonate(self, forceremove=False):
            ...
            if not self.HasSpawnFlags(self.SF_NO_IMPACT_EFFECTS):
                # Normal explosion
                ExplosionCreate( self.vecimpactposition, self.GetAbsAngles(), self, int(50.0), int(500.0), 
    ...
    flighttime = FloatField(value=0.0, keyname='FlightTime', networked=True)
    ...
    horizspeed = FloatField(value=0.0, networked=True)
