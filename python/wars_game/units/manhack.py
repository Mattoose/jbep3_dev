from srcbase import *
from vmath import (Vector, vec3_origin, QAngle, SimpleSplineRemapVal, CrossProduct, VectorNormalize, 
...
class UnitManhack(BaseClass):
    ...
    def __init__(self):
        super(UnitManhack, self).__init__()
    ...
    if isserver:
        def Precache(self):
            ...
            super(UnitManhack, self).Precache()

    def Spawn(self):    
        super(UnitManhack, self).Spawn()
    ...
    if isserver:
        def UpdateOnRemove(self):
            ...
            super(UnitManhack, self).UpdateOnRemove()

        def UnitThink(self):
            super(UnitManhack, self).UnitThink()
        ...
        def StopLoopingSounds(self):
            super(UnitManhack, self).StopLoopingSounds()
            self.enginepitch1 = -1
            self.enginepitch1time = gpGlobals.curtime
            self.enginepitch2 = -1
            self.enginepitch2time = gpGlobals.curtime
        ...
        def VPhysicsCollision(self, index, pEvent):
            super(UnitManhack, self).VPhysicsCollision(index, pEvent)
        ...
        def VPhysicsShadowCollision(self, index, pEvent):
            ...
            super(UnitManhack, self).VPhysicsShadowCollision( index, pEvent )
        ...
        def CheckCollisions(self, flInterval):
            ...
            vecTraceDir = Vector()
            vecCheckPos = Vector()
            self.VPhysicsGetObject().GetVelocity(vecTraceDir, None)
        ...
        def PlayFlySound(self):
            ...
            # Spin sound based on distance from enemy (unless we're crashing)
                ...
                if flEnemyDist < MANHACK_PITCH_DIST1:
                    ...
                    self.enginepitch1 = iPitch1
                    self.enginepitch1time = gpGlobals.curtime + 0.1
                    self.enginepitch2 = iPitch2
        ...
        def Event_Killed(self, info):
            ...
            super(UnitManhack, self).Event_Killed( info )
    ...
    if isclient:
        def OnDataChanged(self, type):
            super(UnitManhack, self).OnDataChanged(type)
        ...
        def OnRestore(self):
            super(UnitManhack, self).OnRestore()
            self.SoundInit()
        ...
        def SoundInit(self):
            ...
            if not self.enginesound1:
                self.enginesound1 = CSoundEnvelopeController.GetController().SoundCreate( filter, self.entindex(), "NPC_Manhack.EngineSound1" )
            ...
            if not self.enginesound2:
                self.enginesound2 = CSoundEnvelopeController.GetController().SoundCreate( filter, self.entindex(), "NPC_Manhack.EngineSound2" )
            ...
            if not self.bladesound:
                self.bladesound = CSoundEnvelopeController.GetController().SoundCreate( filter, self.entindex(), "NPC_Manhack.BladeSound" )
         ...
         class BehaviorGenericClass(BaseClass.BehaviorGenericClass):
            class ActionAttack(BaseClass.BehaviorGenericClass.ActionAttack):
                ...
                def Update(self):
                    ...
                    if self.lastdir and self.outer.mv.blocker == self.outer.enemy:#self.lastmoveaway + 2.0 < gpGlobals.curtime:
                        return self.OnSlice(self.lastdir)
                    
                    return super(BaseClass.BehaviorGenericClass.ActionAttack, self).Update()
                
                def OnSlice(self, dir):
                    self.lastdir = dir
                    #self.lastmoveaway = gpGlobals.curtime
                    return self.SuspendFor(self.behavior.ActionMoveAway, 'Sliced, moving away for new attack', -dir, 0.35)
    ...
    enginepitch1 = IntegerField(value=-1, networked=True)
    enginepitch1time = FloatField(value=0.0, networked=True)
    enginepitch2 = IntegerField(value=-1, networked=True)
    enginepitch2time = FloatField(value=0.0, networked=True)
