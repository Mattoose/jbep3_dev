from srcbase import *
from vmath import Vector, QAngle, matrix3x4_t, vec3_origin, ConcatTransforms, MatrixInvert, MatrixAngles, VectorNormalize, AngleVectors
import random
...
if isserver:
    from entities import SpawnBlood, g_vecAttackDir, CreateRagGib, CreateEntityByName, DispatchSpawn
...
class UnitBaseZombie(BaseClass):
    if isserver:
        def Precache(self):
            super(UnitBaseZombie, self).Precache() 
    ...
    #
    # Output : The entity hit by claws. None if nothing.
    #-----------------------------------------------------------------------------
    def ClawAttack(self, flDist, iDamage, qaViewPunch, vecVelocityPunch, BloodOrigin):
        ...
        if self.enemy:
            tr = trace_t()
            UTIL_TraceHull(self.WorldSpaceCenter(), self.enemy.WorldSpaceCenter(), -Vector(8,8,8), Vector(8,8,8), MASK_SOLID_BRUSHONLY, self, COLLISION_GROUP_NONE, tr)
    ...
    def BecomeRagdoll(self, info, forceVector):
        ...
        if self.istorso or (not self.IsChopped(info) and not self.IsSquashed(info)) or bKilledByVehicle:
            return super(UnitBaseZombie, self).BecomeRagdoll(info, forceVector)
    ...
    def OnTakeDamage_Alive(self, inputInfo):
        ...
        tookDamage = super(UnitBaseZombie, self).OnTakeDamage_Alive(info)
        ...
        if not self.istorso and (bChopped or bSquashed) and not bKilledByVehicle and not (info.GetDamageType() & DMG_REMOVENORAGDOLL):
            ...
            if release == 'RELEASE_IMMEDIATE':
                ...
                self.RemoveHead()
            #elif release == 'RELEASE_SCHEDULED':
            ...
            if self.ShouldBecomeTorso(info, flDamageThreshold):
                ...
                if self.canbecomelivetorso:
                    ...
                    if (info.GetDamageType() & DMG_BLAST) and random.randint( 0, 1 ) == 0:
                        self.Ignite(5.0 + random.uniform(0.0, 5.0))
    ...
    def UpdateTranslateActivityMap(self):
        if self.IsOnFire():
            ...
            return
        super(UnitBaseZombie, self).UpdateTranslateActivityMap()
    ...
    def Ignite(self, flFlameLifetime, bNPCOnly, flSize, bCalledByLevelDesigner):
        super(UnitBaseZombie, self).Ignite( flFlameLifetime, bNPCOnly, flSize, bCalledByLevelDesigner )
