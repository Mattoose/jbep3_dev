...
if isserver:
    ...
    from utils import (UTIL_TraceLine, trace_t, UTIL_SetSize, UTIL_ScreenShake, UTIL_PredictedPosition, SHAKE_START,
...
def CreateDust(origin, angles):
    ...
    for i in range(0, iParticleCount):
        ...
        particle.m_uchColor[0] = int(color[0])
        particle.m_uchColor[1] = int(color[1])
        particle.m_uchColor[2] = int(color[2])
...
class UnitAntlion(BaseClass):
    """ Antlion """
    def __init__(self):
        super(UnitAntlion, self).__init__()
    ...
    def Precache(self):
        if isclient:
            super(UnitAntlion, self).Precache()
            return
    ...
    def CanBecomeRagdoll(self):
        ...
        return super(UnitAntlion, self).CanBecomeRagdoll()    
    ...
    if isserver:
        def StartMeleeAttack(self):    
            ...
            return super(UnitAntlion, self).StartMeleeAttack()
    ...
    if isclient:
        def UpdateOnRemove(self):
            super(UnitAntlion, self).UpdateOnRemove()
    ...
    if isserver:
        ...
        class AntlionWorkerSpit(TossGrenadeAnimEventHandler):
            def HandleEvent(self, unit, event):
                ...
                elif unit.controllerplayer != None:
                    vSpitPos = Vector()
                    unit.GetAttachment( "mouth", vSpitPos )                
    ...
    ANTLIONSUICIDER_BURSTDAMAGE = 25.0
    ANTLIONSUICIDER_BURSTRADIUS = 64.0
    dontexplode = False
    ...
    burrowtexture = None
    burrowed = BooleanField(value=False, networked=True, clientchangecallback='OnBurrowedChanged')
    ...
    class AnimStateClass(BaseClass.AnimStateClass):
        ...
        def __init__(self, outer, animconfig):
            super(UnitAntlion.AnimStateClass, self).__init__(outer, animconfig)
            
            self.isburrowed = False
            
        def OnNewModel(self):
            super(UnitAntlion.AnimStateClass, self).OnNewModel()
        ...
        def OnEndSpecificActivity(self, specificactivity):
            ...
            return super(UnitAntlion.AnimStateClass, self).OnEndSpecificActivity(specificactivity)
    ...
    if isserver:
        class BehaviorGenericClass(BaseClass.BehaviorGenericClass):
            ...
            class ActionBurrow(BaseAction):
                def OnStart(self):
                    self.outer.DoAnimation(self.outer.ANIM_STARTBURROW)
                    self.outer.aimoving = True
                    self.outer.burrowed = True
                    self.outer.AddSolidFlags(FSOLID_NOT_SOLID)
                    self.outer.SetCanBeSeen(False)
            
                def OnEnd(self):
                    self.outer.takedamage = DAMAGE_YES
                    self.outer.aimoving = False
                    self.outer.burrowed = False
                    #self.outer.RemoveEffects(EF_NODRAW)
                    self.outer.SetCanBeSeen(True)
    ...
    class AttackLeap(UnitInfo.AttackBase):
        ...
        def CanAttack(self, unit, enemy):
            if self.nextattacktime > gpGlobals.curtime:
                return False
            return unit.CanRangeAttack(enemy.WorldSpaceCenter())
        def Attack(self, unit, enemy): 
            self.nextattacktime = gpGlobals.curtime + 6.0
            return unit.StartLeapAttack()
...
class AntlionInfo(AntlionInfoShared):
    ...
    image_name = 'vgui/units/unit_antlion.vmt'
