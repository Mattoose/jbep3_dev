...
from core.resources import UpdateResource, RESOURCE_GRUB
...
class AntlionGrub(UnitBase):
    def __init__(self):
        super(AntlionGrub, self).__init__()
    ...
    def UpdateOnRemove(self):
        ...
        if self.isresourceactive:
            # Remove one grub
            UpdateResource(self.GetOwnerNumber(), RESOURCE_GRUB, -1)
            if self.GetOwnerEntity():
                try: self.GetOwnerEntity().GrubDied(self)
                except AttributeError: pass
        
        # ALWAYS CHAIN BACK!
        super(AntlionGrub, self).UpdateOnRemove()
    ...
    def Event_Killed(self, info):
        ...
        # we deliberately do not call super(AntlionGrub, self).EventKilled
    ...
    def OnTakeDamage(self, info):
        ...
        return super(AntlionGrub, self).OnTakeDamage( info )

    def Spawn(self):
        super(AntlionGrub, self).Spawn()
    ...
    def Activate(self):
        super(AntlionGrub, self).Activate()
    ...
    def OnChangeOwnerNumber(self, old_owner_number):
        super(AntlionGrub, self).OnChangeOwnerNumber(old_owner_number)

        if isserver:
            if self.isresourceactive:
                # Old one decrement one grub, new one increment one grub
                UpdateResource( old_owner_number, RESOURCE_GRUB, -1)
                UpdateResource( self.GetOwnerNumber(), RESOURCE_GRUB, 1)
    ...
    def Precache(self):
        ...
        super(AntlionGrub, self).Precache()
    ...
    def TraceAttack(self, info, vecDir, tr):
        ...
        super(AntlionGrub, self).TraceAttack( info, vecDir, tr )
