...
if isserver:
    from entities import (CBaseGrenade as BaseClass, CreateEntityByName, DispatchSpawn, CTakeDamageInfo,
                          CSoundEnt, SOUND_DANGER, EFL_FORCE_CHECK_TRANSMIT, Class_T, RadiusDamage)
    from utils import UTIL_SetSize, UTIL_ScaleForGravity, UTIL_Remove, UTIL_DecalTrace
    ...
    class GrenadeSpit(BaseClass):
        def __init__(self):
            super(GrenadeSpit, self).__init__()
            self.playsound = True
            self.hissSound = None
        ...
        def Spawn(self):
            ...
            #self.spiteffect = CreateEntityByName( "info_particle_system" )
            self.spiteffect = None
            if self.spiteffect != None:
                # Setup our basic parameters
                self.spiteffect.KeyValue( "start_active", "1" )
                self.spiteffect.KeyValue( "effect_name", "antlion_spit_trail" )
                self.spiteffect.SetParent( self )
                self.spiteffect.SetLocalOrigin( vec3_origin )
                DispatchSpawn( self.spiteffect )
                if gpGlobals.curtime > 0.5:
                    self.spiteffect.Activate()
        ...
        def GrenadeSpitTouch(self, pOther):
            ...
            #poisonratio = self.GRENADE_POISON_RATIO
        ...
        def Detonate(self):
            ...
            info = CTakeDamageInfo( self, self.GetThrower(), self.damage * (1.0-poisonratio), self.damagetype )
            RadiusDamage( info, self.GetAbsOrigin(), self.damageradius, Class_T.CLASS_NONE, self.GetOwnerEntity() )
            info = CTakeDamageInfo( self, self.GetThrower(), self.damage * poisonratio, DMG_POISON )
            RadiusDamage( info, self.GetAbsOrigin(), self.damageradius, Class_T.CLASS_NONE, self.GetOwnerEntity() )
