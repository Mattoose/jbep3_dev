...
class BuildBridge(BaseClass):
    def __init__(self):
        super(BuildBridge, self).__init__()
        
        self.navids = []
        
    if isserver:
        def Precache(self):
            super(BuildBridge, self).Precache()
            
            self.PrecacheModel(self.MODEL_BROKEN)
            self.PrecacheModel(self.MODEL_HEALTHY)
            
        def Spawn(self):
            super(BuildBridge, self).Spawn()
        ...
        def UpdateOnRemove(self):
            # ALWAYS CHAIN BACK!
            super(BuildBridge, self).UpdateOnRemove()
        ...
        def DestroyNavAreas(self):
            map(DestroyNavArea, self.navids)
            self.navids = []
