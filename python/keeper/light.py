...
def CreateDungeonLight(origin, yaw=0, pitch=70):
    ...
    try:
        deferredlighting = bool(int(engine.GetClientConVarValue(1, 'deferred_lighting')))
    except:
        deferredlighting = False
        
    #print engine.GetClientConVarValue(1, 'deferred_lighting')
    #print 'Creating dungeon light at %s (deferred lighting: %s)' % (str(origin), str(deferredlighting))
    #ndebugoverlay.Box(origin, -Vector(4, 4, 4), Vector(4, 4, 4), 0, 255, 0, 255, 10.0)
