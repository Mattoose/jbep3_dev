...
if isclient:
    from vgui import cursors, surface, scheme, Vertex_t, FontDrawType_t
    ...
    class StatusBalloonPanel(Panel):
        def __init__(self):
            ...
            self.statustextures = {}
            for k, v in statusicons.iteritems():
        ...
        def Paint(self):
            ...
            drawpoints = [
                Vertex_t(Vector2D(0,0), Vector2D(0,1)),
                Vertex_t(Vector2D(self.GetWide(),0) , Vector2D(0,0)),
                Vertex_t(Vector2D(self.GetWide(),self.GetTall()), Vector2D(1,0)),
                Vertex_t(Vector2D(0,self.GetTall()), Vector2D(1,1)),
...
class UnitBaseKeeper(BaseClass):
    ...
    def FindTrainRoom(self):
        ...
        if not trainrooms:
            return None
        trainrooms = filter(lambda r: not r.IsFull(), trainrooms)
    ...
    if isserver:
        ...
        def PerformGrabbedLocomotion(self):
            ...
            self.SetSimulationTime(gpGlobals.curtime)
...
class UnitBaseCreature(UnitBaseKeeper):
    ...
    def FindLair(self):
        freelairs = filter(lambda l: not l.attachedlair, tilespertype[self.GetOwnerNumber()]['lair'])
        if not freelairs:
            return None
...
class UnitKeeperInfo(UnitInfo):
    ...
    traincost = 1 # Cost per hit when training
