...
class BehaviorKeeper(BehaviorGeneric):
    ...
    class ActionMoveToBlock(ActionMoveToTile):
        ...
        def ComputeTargetTile(self):
            ...
            besttile = None
            bestdist = None
            for tile in self.block.neighbors.itervalues():
...
class BehaviorImp(BehaviorKeeper):
    ...
    class ActionConvertTile(BehaviorKeeper.ActionTask):
        ...
        def Update(self):
            if not self.tile or not self.tile.IsClaimable(self.outer.GetOwnerNumber()):
                return self.Done('Tile changed.')
        
            pos = keeperworld.keeperworld.GetPosFromKey(self.tile.key)
            dist = (pos - self.outer.GetAbsOrigin()).Length2D()
            if dist > 32.0:
                return self.SuspendFor(self.behavior.ActionMoveTo, "Moving to tile", pos, tolerance=12.0) 
                
            self.creatingtile = True
            self.tile.impassigned = self.outer.GetHandle()
        ...
        def OnResume(self):
            ...
            if self.creatingtile:
                DispatchParticleEffect('grub_death_juice', PATTACH_ABSORIGIN, self.outer)
                self.outer.EmitSound('Imp.ConvertTile')
                self.tile.Claim(self.outer.GetOwnerNumber())
...
class BehaviorCreature(BehaviorKeeper):
    ...
    def GetSortedActivities(self):
        ...
        return sorted(sortedacts.iteritems(), key=operator.itemgetter(1))
    ...
    class ActionIdle(BehaviorKeeper.ActionIdle):
        def Update(self):
            ...
            if self.outer.paypending and self.outer.CanBePayed():
    ...
    class ActionEat(BaseAction):
        ...
        def Update(self):
            if not self.hatchery:
                self.hatchery = self.outer.FindHatchery()
            ...
            dist = (pos - self.outer.GetAbsOrigin()).Length2D()
            if dist > 32.0:
                return self.SuspendFor(self.behavior.ActionMoveTo, "Moving to grub in hatchery", pos, tolerance=12.0) 
                
            self.eating = True
            #grub.Remove() # Eat the grub!
            self.grub.health = 0
            info = CTakeDamageInfo(self.outer, self.outer, 0, 0)
