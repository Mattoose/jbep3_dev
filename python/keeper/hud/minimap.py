...
from vgui import CHudElement, GetClientMode, scheme, surface, Vertex_t
...
class Minimap(CHudElement, BaseHudMinimap):
    ...
    def DrawTiles(self):
        ...
        drawpoints = [
            Vertex_t(Vector2D(0,0), Vector2D(0,self.coordy)),
            Vertex_t(Vector2D(self.GetWide(),0) , Vector2D(self.coordx,self.coordy)),
            Vertex_t(Vector2D(self.GetWide(),self.GetTall()), Vector2D(self.coordx,0)),
            Vertex_t(Vector2D(0,self.GetTall()), Vector2D(0,0)),
        ]
