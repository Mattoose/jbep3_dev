from srcbase import FL_STATICPROP, MOVETYPE_NONE, SOLID_BBOX, EF_NODRAW, DONT_BLEED, Color
from vmath import Vector, QAngle, anglemod
import srcmgr
from entities import IMouse, EFL_SERVER_ONLY, DENSITY_NONE
import ndebugoverlay 
from _navmesh import NavMeshGetPathDistance, GetNavAreaAt, TryMergeSurrounding
from fow import FogOfWarMgr
from gameinterface import modelinfo, engine, ConVar, FCVAR_CHEAT
...
if isclient:
    ...
    if srcmgr.steamappid != 630:
        from entities import RENDER_GROUP_OPAQUE_ENTITY
    from srcbase import RenderMode_t
else:
    from entities import CBaseAnimating as BaseClass
    from _navmesh import CreateNavArea, DestroyNavArea
...
def SpawnTile(cls, owner, key, origin, ownernumber=1):
    ...
    if isclient:
        ...
        if srcmgr.steamappid != 630 and tile.InitializeAsClientEntity(tile.modelname, RENDER_GROUP_OPAQUE_ENTITY):
            tile.SetAbsOrigin(origin)
            tile.Spawn()
        elif srcmgr.steamappid == 630 and tile.InitializeAsClientEntity(tile.modelname, False):
            tile.SetAbsOrigin(origin)
            tile.Spawn()
        else:
...
def GenerateModelCases(models):
    ...
    generatetestbits = models.keys()
    for testbits in generatetestbits:
        yaw = anglemod(models[testbits][1])
        ...
        for i in range(0, 3):
            ...
            if newtestbits not in models:
                models[newtestbits] = (models[testbits][0], yaw)
...
class TileBase(BaseClass, IMouse):
    ...
    def UpdateModel(self):
        ...
        modelname = None
        if len(testbits) != 4:
            print('UpdateModel: invalid test bit string length')
        else:
            if testbits in self.models:
                modelname, yawoffset = self.models[testbits]
                self.SetAbsAngles(QAngle(0, yawoffset, 0))
    ...
    def Spawn(self):
        ...
        if self.isblock:
            FogOfWarMgr().ModifyHeightAtExtent(mins, maxs, 10000.0)
        else:
    ...
    def UpdateOnRemove(self):
        if self.owner.worldisremoving:
            super(TileBase, self).UpdateOnRemove()
            return
    ...
    def OnNeighborChanged(self, othertile):
        ...
        if not self.isblock and not othertile.isblock:
            if self not in othertile.connectedareas:
    ...
    def IsReachable(self, unit):
        ...
        for tile in self.neighbors.itervalues():
            if not tile or tile.isblock: 
                continue
            if unittile.connectedareas != tile.connectedareas:
                continue
            #if NavMeshGetPathDistance(unit.GetAbsOrigin(), tile.GetAbsOrigin(), anyz=True) < 0:
            #    continue
            return True
    ...
    def MergeAreaSet(self, areas):
        ...
        if taskqueues != None:
            for owner in taskqueues.iterkeys():
...
class TileGroundBase(TileBase):
    ...
    def Spawn(self):
        super(TileGroundBase, self).Spawn()
        ...
        if isserver:
            ...
            if self.createnavarea:
                ...
                if GetNavAreaAt(origin) == -1:
                    # Create a nav area
                    unbloat = 4.0
                    corner = origin - Vector(owner.xsize/2.0 - unbloat, owner.ysize/2.0 - unbloat, 0.0)
                    othercorner = origin + Vector(owner.xsize/2.0 - unbloat, owner.ysize/2.0 - unbloat, 0.0)
                    navid = CreateNavArea(corner, othercorner)
                    TryMergeSurrounding(navid, 8.0)
            
    def UpdateOnRemove(self):
        if isserver and self.navid != -1:
            #DestroyNavArea(self.navid)
            self.navid = -1
    
        # ALWAYS CHAIN BACK!
        super(TileGroundBase, self).UpdateOnRemove()
    ...
    def OnNeighborChanged(self, othertile):
        ''' Updates neighbor information. '''
        super(TileGroundBase, self).OnNeighborChanged(othertile)
...
class PortalTile(TileGroundBase):
    ...
    if isserver:
        def Activate(self):
            super(PortalTile, self).Activate()
