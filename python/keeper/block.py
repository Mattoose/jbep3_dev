...
class BlockBase(TileBase):
    ...
    def DebugPrint(self):
        return super(BlockBase, self).DebugPrint() + ', hp: %d' % (self.health)
    ...
    def UpdateOnRemove(self):
        ...
        # ALWAYS CHAIN BACK!
        super(BlockBase, self).UpdateOnRemove()
    ...
    def OnTakeDamage(self, dmginfo):
        ...
        return super(BlockBase, self).OnTakeDamage(dmginfo)
        
    def Event_Killed(self, info):
        self.Cleanup()
        #return super(Block, self).Event_Killed(info)
    ...
    def OnCursorEntered(self, player):
        super(BlockBase, self).OnCursorEntered(player)
    ...
    def OnCursorExited(self, player):
        super(BlockBase, self).OnCursorExited(player)
    ...
    def MarkSelected(self):
        if isclient:
            ...
            #self.skin = 1
            #if self.glowhandle == None:
            #    self.glowhandle = glowobjectmanager.RegisterGlowObject(self, Vector(0, 0, 1.0), 1.0, True, True, -1)
            #    print('#%d Glowing %d' % (self.entindex(), self.glowhandle))
            #    glowobjectmanager.SetFullBloomRender(self.glowhandle, True, 1)
    ...
    def UnmarkSelected(self):
        if isclient:
            ...
            #if self.glowhandle >= 0:
            #    glowobjectmanager.UnregisterGlowObject(self.glowhandle)
    ...
    def OnNeighborChanged(self, othertile):
        super(BlockBase, self).OnNeighborChanged(othertile)
    ...
    def UpdateDigTaskIfSelected(self):
        ...
        if not self.HasOneOrMoreNonBlockNeighbors():
            for tqownernumber, tq in taskqueues.iteritems():
                if tqownernumber >= 1:
...
class Block(BlockBase):
    ...
    models = {
        '0000' : ('models/keeper/infested_pillar.mdl', 0.0), # In the middle of non block tiles
        '1000' : ('models/keeper/infested_endwall.mdl', 0.0),
        '1100' : ('models/keeper/infested_corner.mdl', 180.0), # Corner
        '1110' : ('models/keeper/infested_1sided.mdl', 180.0),
        '1010' : ('models/keeper/infested_2sided.mdl', 180.0),
        '1111' : ('models/keeper/infested_cap.mdl', 0.0), # Enclosed by block tiles
    }
    GenerateModelCases(models) # Updates the dictionary with all cases
    
    def DebugPrint(self):
        return super(Block, self).DebugPrint() + ', fortified: %s, testbits: %s' % (self.fortified, self.GenerateTestBitString())
        
    def UpdateOnRemove(self):
        self.ClearLight()
        
        # ALWAYS CHAIN BACK!
        super(Block, self).UpdateOnRemove()
        
    def OnNeighborChanged(self, othertile):
        super(Block, self).OnNeighborChanged(othertile)
    ...
    def UpdateDigTaskIfSelected(self):
        super(Block, self).UpdateDigTaskIfSelected()
...
class BlockGold(BlockBase):
    ...
    def UpdateModel(self):
        super(BlockGold, self).UpdateModel()
    ...
    if isserver:
        ...
        def OnTakeDamage(self, dmginfo):
            dmg = super(BlockGold, self).OnTakeDamage(dmginfo)
        ...
        def Event_Killed(self, info):
            rv = super(BlockGold, self).Event_Killed(info)
