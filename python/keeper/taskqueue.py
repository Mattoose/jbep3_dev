...
@usermessage()
def ClientSetMaxGold(maxgold, **kwargs):
    gamerules.maxgold = maxgold
...
class Task(object):
    ...
    def UpdateTaskAvailability(self):
        ...
        self.imps = filter(None, self.imps)
...
class TaskQueue(object):
    ...
    def FindNearestFreeImp(self, key, freeimps=None):
        ...
        impkeys = map(lambda imp: imp.key, freeimps)
    ...
    def UpdateAllTaskAvailability(self):
        map(lambda t: t.UpdateTaskAvailability(), self.tasks)
    ...
    def UpdateMaxGold(self):
        ...
        #print('Setting max gold: %s for %s' % (self.maxtotalgold, str(players)))
        ClientSetMaxGold(self.maxtotalgold, filter=filter)
    ...
    def Update(self):
        ...
        if not self.freeimps or not self.tasks or not self.keytofreetasks.keys():
    ...
    def UpdateAllTasks(self):
        ...
        map(self.FindTaskForImp, self.freeimps)
...
if isserver:
    ...
    def CCForceUpdateTasks(args):
        for tq in taskqueues.itervalues():
            tq.UpdateAllTasks()
