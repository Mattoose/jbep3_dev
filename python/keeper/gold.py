...
from core.resources import resource_types, UpdateResource, resources, ResetResource
...
def GoldTakeResources(ownernumber, resource):  
    goldvalue = resource[1]
...
def GoldGiveResources(ownernumber, resource): 
    ...
    if goldvalue > 0:
        PrintWarning('GoldGiveResources: Failed to add gold to treasure tiles of owner %d (already filled or no tiles). Remainder: %d\n' % (ownernumber, goldvalue))

def LoadDKResources():
    resource_types['gold'] = (GoldTakeResources, GoldGiveResources)
...
class Gold(PickupableObject, CBaseAnimating):
    ...
    def UpdateOnRemove(self):
        ...
        # ALWAYS CHAIN BACK!
        super(Gold, self).UpdateOnRemove()
        
    def OnChangeOwnerNumber(self, oldownernumber):
        super(Gold, self).OnChangeOwnerNumber(oldownernumber)
    ...
    def UpdateGoldState(self):
        ...
        if not self.goldactive:
            ...
            for ownernumber, tq in taskqueues.iteritems():
