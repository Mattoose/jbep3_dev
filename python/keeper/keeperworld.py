from srcbase import (SOLID_CUSTOM, FSOLID_CUSTOMRAYTEST, FSOLID_CUSTOMBOXTEST, LIFE_ALIVE, 
                     DAMAGE_YES, COLLISION_GROUP_NONE, MASK_SOLID, RegisterTickMethod)
from vmath import Vector, vec3_origin, vec3_angle, QAngle, VectorNormalize
import srcmgr
...
from fields import ListField, StringField, BooleanField, GenericField, IntegerField
...
from playermgr import OWNER_NEUTRAL, OWNER_ENEMY
from core.usermessages import usermessage
from core.signals import prelevelshutdown, FireSignalRobust
from core.units import PrecacheUnit
from particles import *
from signals import keeperworldloaded
from _navmesh import DestroyAllNavAreas
...
if isserver:
    ...
    from _navmesh import CreateNavArea, NavMeshGetPathDistance
    ...
    from entities import C_BaseEntity as BaseClass, DataUpdateType_t
...
def ClientFortifyBlock(key, **kwargs):
    keeperworld.tilegrid[key].Fortify()
...
class KeeperWorld(BaseClass):
    def __init__(self):
        ...
        super(KeeperWorld, self).__init__()
        
        self.tilegrid = defaultdict(lambda : Nothing())

        print('======================= INIT INIT INIT INIT INIT KeeperWorld %s: Setting refs' % (isclient))
        keeperworld = self
    ...
    def Precache(self):
        super(KeeperWorld, self).Precache()
        ...
        for k, v in tilemap.iteritems():
            self.PrecacheModel(v.modelname)
            for modelname, offset in v.models.itervalues():
    ...
    if isserver:
        def Spawn(self):
            ...
            super(KeeperWorld, self).Spawn()
            
            self.LoadMap((nextlevel, random.randint(0, sys.maxint)))
            nextlevel = ''
    ...
    def UpdateOnRemove(self):
        ...
        # ALWAYS CHAIN BACK!
        super(KeeperWorld, self).UpdateOnRemove()
        
        print('======================= SHUTDOWN SHUTDOWN SHUTDOWN SHUTDOWN KeeperWorld %s: clearing refs' % (isclient))
        if keeperworld == self:
            keeperworld = None
    ...
    def ClearTileGrid(self):
        ...
        for b in self.tilegrid.itervalues():
            if not b:
                continue
            try:
                b.Remove()
    ...
    def LoadMap(self, levelsettings=None):
        ...
        if not levelsettings:
            assert(isserver)
            levelname = None
            randomlevelseed = random.randint(0, sys.maxint)
    ...
    def LoadDelayed(self):  
        ...
        map(lambda tile: tile.Activate(), self.tilegrid.values())
    ...
    def CreateFromMap(self, blockmap):
        tiles = []
        for key, entry in blockmap.iteritems():
            tile = self.CreateTile(entry['type'], key, entry['ownernumber'], activate=False)
            if 'sellable' in entry:
                tile.sellable = entry['sellable']
            tiles.append(tile)
            
        map(lambda tile: tile.Activate(), tiles)
    ...
    def CreateRouteToHeart(self, key, maxdist=10000):
        ...
        for i in range(0, maxdist):
            ...
            for ntile in tile.neighbors.itervalues():
...
if isserver:
    ...
    def CCPrintConnectedAreas(args):
        # Collect areas
        areaslists = []
        for tile in keeperworld.tilegrid.itervalues():
