...
class Label(Panel):
    ...
    def SetText(self, text):
        ...
        if text and text[0] == '#':
            self.SetHotkey(self.CalculateHotkey(localize.Find(text)))
        else:
    ...
    def OnSizeChanged(self, wide, tall):
        ...
        self.InvalidateLayout()
        Panel.OnSizeChanged(self, wide, tall)
    ...
    def RequestInfo(self, outputData):
        ...
        return Panel.RequestInfo(self, outputData)
    ...
    def GetSettings(self, outResourceData):
        # panel settings
        Panel.GetSettings(self, outResourceData)
    ...
    def ApplySettings(self, inResourceData):
        Panel.ApplySettings( self, inResourceData )

        # label settings
        labelText =	inResourceData.GetString( "labelText", None )
        if labelText:
            if labelText[0] == '%' and labelText[strlen(labelText) - 1] == '%':
