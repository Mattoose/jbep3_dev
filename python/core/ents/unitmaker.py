...
from fields import FloatField, StringField, IntegerField, GenericField, OutputField, FlagsField, input
import hull
...
class CNPCSpawnDestination(CPointEntity):
    ...
    onspawnnpc = OutputField(keyname='OnSpawnNPC')
...
class CBaseNPCMaker(CBaseEntity):
    ...
    disabled = IntegerField(keyname='StartDisabled')
    ...
    ignoreentity = GenericField()
    ingoreent = StringField(keyname='IgnoreEntity')
    
    ownernumber = IntegerField(value=3, keyname='OwnerNumber', nofgd=True)
    ...
    spawnflags = FlagsField(keyname='spawnflags', flags=
        [('SF_NPCMAKER_START_ON', ( 1 << 0 ), False), # start active ( if has targetname )
         ...
         ('SF_NPCMAKER_FADE', ( 1 << 4 ), False), # Children's corpses fade
         ('SF_NPCMAKER_INF_CHILD', ( 1 << 5 ), False), # Infinite number of children
         ('SF_NPCMAKER_NO_DROP', ( 1 << 6 ), False), # Do not adjust for the ground's position when checking for spawn
         ('SF_NPCMAKER_HIDEFROMPLAYER', ( 1 << 7 ), False), # Don't spawn if the player's looking at me
        ...
        if self.HasSpawnFlags(self.SF_NPCMAKER_HIDEFROMPLAYER):
            for i in range(1, gpGlobals.maxClients+1):
                ...
                if pPlayer:
                    ...
                    if pPlayer.FInViewCone( GetAbsOrigin() ) and pPlayer.FVisible(self.GetAbsOrigin()):
    ...
    # Purpose: 
    #-----------------------------------------------------------------------------
    @input(inputname='SetMaxChildren')
    ...
    # Purpose: 
    #-----------------------------------------------------------------------------
    @input(inputname='AddMaxChildren')
...
class CNPCMaker(CBaseNPCMaker):
    ...
    #-----------------------------------------------------------------------------
    def MakeNPC(self):
        if not self.CanMakeNPC():
            return

        pent = CreateUnitNoSpawn(self.npcclassname, self.ownernumber)
        if not pent:
        ...
        pent.SetAbsOrigin(self.GetAbsOrigin())
