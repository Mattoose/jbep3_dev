...
class AbilityButton(Button):
    ...
    def ApplySchemeSettings(self, scheme):
        super(AbilityButton, self).ApplySchemeSettings(scheme)
    ...
    def SetEnabled(self, enabled):
        super(AbilityButton, self).SetEnabled(enabled)
        self.RecalculateCurrentImage()
            
    def SetArmed(self, state):
        super(AbilityButton, self).SetArmed(state)
        self.RecalculateCurrentImage()

    def RecalculateDepressedState(self):
        super(AbilityButton, self).RecalculateDepressedState()
        self.RecalculateCurrentImage()
    ...
    def Paint(self):    
        ...
        if self.currentimage:
            self.currentimage.DrawSelf(0, 0, w, h, Color(255,255,255,255))
        if self._iconimage:
        ...
        if self.currentoverlayimage:
            self.currentoverlayimage.DrawSelf(0, 0, w, h, Color(255,255,255,255))

        super(AbilityButton, self).Paint()
