...
try:
    ...
    print('core.utils.GenerateImage: Couldn\'t retrieve STEAMUSER')
    SDKDIR = ''
...
try:
    ...
    print('core.utils.GenerateImage: Set IMAGEMAGICK in your environ variables to the executable of Image Magick')
...
def GenerateImage(info, targetfolder, targetoutputfolder, nosubcat=False):
    ...
    #cmdline = [IMAGEMAGICKEXE, '-size', '128x128', 'canvas:none', '-pointsize', '56', '-draw', '"text 20,80 \'%s\'"' % (abbr), RelToAbs('%s.png' % (info.name))]
...
def CCGenerateStubIcons(args):
    ...
    for faction in dbfactions.itervalues():
        ...
        targetfolder = RelToAbs('materialsrc/vgui/%s/' % (faction.name))
...
def CCGenerateSingleStubIcon(args):
    ...
    targetfolder = RelToAbs('materialsrc/%s' % (args[2]))
