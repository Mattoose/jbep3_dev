...
class BaseField(object):
    ...
    def InitField(self, inst):
        ...
        if self.networked:
            if not self.propname:
                NetworkVar(inst, self.name, self.default, changedcallback=self.clientchangecallback)
            else:
    ...
    def GenerateFGDProperty(self):
        if self.choices:
            entry = '%s(choices) : "%s" : %s : "%s" =\n\t[\n' % (self.name, self.displayname, self.GenerateFGDDefaultValue(), self.helpstring)
    ...
    def InitField(self, inst):
        if self.networked:
            ...
            NetworkVar(inst, self.name, Vector(self.default), 
                    changedcallback=self.clientchangecallback)
        else:
        ...
        if self.networked:
            ...
            if self.defaultvalue is None:
                ...
                NetworkDefaultDict(inst, self.name, dict(self.default),
                        default=self.defaultvalue, changedcallback=self.clientchangecallback) 
        else:
...
class FlagsField(BaseField):
    def __init__(self, value=0, flags=[], *args, **kwargs):
        self.flags = flags
        for flagname, value, defaultvalue in self.flags: 
            value |= defaultvalue
...
output_fgdtypes = {
    ...
    fieldtypes.FIELD_BOOLEAN : 'boolean',
...
class OutputField(BaseField):
    ...
    def __init__(self, keyname, fieldtype=fieldtypes.FIELD_VOID):
        super(OutputField, self).__init__(keyname=keyname, noreset=True)
        self.fieldtype = fieldtype
    ...
    def InitField(self, unit):
        ...
        if abiinfo:
            ...
            if technode.techenabled:
                if self.networked:
                    NetworkVar(unit, self.name, technode.upgradevalue, changedcallback=self.clientchangecallback).NetworkStateChanged()
                else:
                    setattr(unit, self.name, technode.upgradevalue)
            elif prevabiinfo:
                technode = prevabiinfo.GetTechNode(prevabiinfo.name, unit.GetOwnerNumber())
                if self.networked:
                    NetworkVar(unit, self.name, technode.upgradevalue).NetworkStateChanged()
                else:
                    setattr(unit, self.name, technode.upgradevalue)
        elif prevabiinfo:
            technode = prevabiinfo.GetTechNode(prevabiinfo.name, unit.GetOwnerNumber())
...
def SetupClassFields(cls, done=None):
    ...
    for basecls in cls.__bases__:
        try:
            basekeyvaluemap = dict(getattr(basecls, 'keyvaluemap'))
            keyvaluemap.update(basekeyvaluemap)
        except:
            pass
    keyvaluemap.update(keyfields)
    keyvaluemap = dict(filter(lambda x: not x[1].cppimplemented, keyvaluemap.items()))
